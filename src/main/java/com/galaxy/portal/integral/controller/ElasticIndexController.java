...
import com.galaxy.portal.elasticsearch.entiy.ResponseCode;
import com.galaxy.portal.elasticsearch.entiy.ResponseResult;
import com.galaxy.portal.elasticsearch.service.BaseElasticService;
import com.galaxy.portal.elasticsearch.vo.IdxVo;
...
public class ElasticIndexController {
    ...
    public ResponseResult createIndex(@RequestBody IdxVo idxVo){
        ...
        try {
            ...
            response.setStatus(false);
            response.setCode(500);
            response.setMsg("error 发生错误" + e.getMessage());
        }
        return response;
    }
    ...
    @GetMapping(value="/indexContainField/{index}/{field}")
    public ResponseResult indexContainField(@PathVariable(value = "index")String index,@PathVariable(value = "field")String field){
        ResponseResult response = new ResponseResult();
        try {
            final boolean b = baseElasticService.indexContainField(index, field);
        } catch (Exception e) {
            e.printStackTrace();
            response.setCode(ResponseCode.NETWORK_ERROR.getCode());
            response.setMsg(" 调用ElasticSearch 失败！");
